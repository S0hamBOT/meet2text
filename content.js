...
const extensionStatusJSON_bug = {
  "status": 400,
  "message": `<strong>Meet2Text encountered a new error</strong> <br /> Please report it <a href="https://github.com/S0hamBOT/transcriptonic/issues" target="_blank">here</a>.`
}

const reportErrorMessage = "There is a bug in Meet2Text. Please report it at https://github.com/S0hamBOT/transcriptonic/issues"
  ...
  waitForElement(meetingEndIconData.selector, meetingEndIconData.text).then(() => {
    ...
    if (!isTranscriptDomErrorCaptured && !isChatMessagesDomErrorCaptured) {
      chrome.storage.sync.get(["operationMode"], function (resultSyncUntyped) {
        const resultSync = /** @type {ResultSync} */ (resultSyncUntyped)
        if (resultSync.operationMode === "manual") {
          showNotification({ status: 400, message: "<strong>Meet2Text is not running</strong> <br /> Turn on captions using the CC icon, if needed" })
        }
        else {
          showNotification(extensionStatusJSON)
        }
...
function pulseStatus() {
  ...
  let activityStatus = document.querySelector(`#Meet2Text-status`)
  if (!activityStatus) {
    let html = document.querySelector("html")
    activityStatus = document.createElement("div")
    activityStatus.setAttribute("id", "Meet2Text-status")
    activityStatus.style.cssText = `background: linear-gradient(90deg, #4285f4, #34a853, #fbbc04, #ea4335); ${statusActivityCSS}`
  ...
  setTimeout(() => {
    activityStatus.style.cssText = `background: transparent; ${statusActivityCSS}`
  }, 2500)
}
...
function showNotification(extensionStatusJSON) {
  ...
  logo.setAttribute("height", "42px")
  logo.setAttribute("width", "42px")
  logo.style.cssText = "border-radius: 12px; box-shadow: 0 2px 8px rgba(66, 133, 244, 0.2); border: 2px solid rgba(255, 255, 255, 0.8);"

  // Remove banner after 6s
  setTimeout(() => {
    obj.style.cssText = obj.style.cssText + "; opacity: 0; transform: translateY(-20px);"
    setTimeout(() => {
  ...
  if (extensionStatusJSON.status === 200) {
    obj.style.cssText = `color: #1a73e8; ${commonCSS}`
    text.innerHTML = extensionStatusJSON.message
  }
  else {
    obj.style.cssText = `color: #ea4335; ${commonCSS}`
    text.innerHTML = extensionStatusJSON.message
  }
...
const commonCSS = `background: rgba(255, 255, 255, 0.95); 
...
function checkExtensionStatus() {
  return new Promise((resolve, reject) => {
    ...
    chrome.storage.local.set({
      extensionStatusJSON: { status: 200, message: "<strong>Meet2Text is running</strong> <br /> Do not turn off captions" },
    })
