    ...
    return new Promise((resolve, reject) => {
        chrome.storage.local.get(["meetings"], function (resultLocalUntyped) {
            ...
            if (resultLocal.meetings && resultLocal.meetings[index]) {
                ...
                const fileName = `Meet2Text/Transcript-${sanitisedMeetingTitle} at ${formattedTimestamp}.txt`
                ...
                // Add branding
                content += "\n\n---------------\n"
                content += "Transcript saved using Meet2Text extension."
                content += "\n---------------"
                    ...
                    if (event.target?.result) {
                        ...
                        chrome.downloads.download({
                            ...
                            chrome.downloads.download({
                                // @ts-ignore
                                url: dataUrl,
                                filename: "Meet2Text/Transcript.txt",
                                conflictAction: "uniquify"
                            })
                            console.log("Invalid file name. Transcript downloaded to Meet2Text directory with simple file name.")
